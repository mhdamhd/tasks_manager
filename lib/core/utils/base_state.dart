...
class BaseState<T> extends Equatable {
  ...
  BaseState<T> updating() {
    return copyWith(requestStatus: RequestStatus.updating);
  }

  // Returns a new BaseState object with the request status set to loading
  BaseState<T> loading({T? newData}) {
    return copyWith(requestStatus: RequestStatus.loading, data: newData);
  }

  BaseState<T> updateSuccess(T newData) {
    return copyWith(requestStatus: RequestStatus.updateSuccess, data: newData);
  }
  ...
  BaseState<T> updateError(Failure newFailure) {
    return copyWith(requestStatus: RequestStatus.updateError, failure: newFailure);
  }
  ...
  bool get isUpdating => requestStatus == RequestStatus.updating;
  ...
  bool get isUpdateSuccess => requestStatus == RequestStatus.updateSuccess;
  ...
  bool get isUpdateError => requestStatus == RequestStatus.updateError;
